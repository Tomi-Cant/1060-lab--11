    ...
    For `void Maze::showMaze(){`, we will do things in a similar way as in the creation of the maze map, make sure it is spaced correctly. For a 3x3 grid it should look like this (given your empty space is '*'):
    ...
    *  *  |  *  *  
    ...
    ``` C++
    wall validation check 1: false 
    wall validation check 2: false
    wall validation check 3: false
    wall validation check 4: true
    wall validation check 5: false
    ```
    ...
    **_Show your TA your work, then switch your driver and navigator roles before
    ...
    Now we'll finish up the missing code. In the move function, there is one line that needs to be added to it when the player successfully moves which is a push back to the moves vector. Since we are now adding to the vector we also need a way to remove from it too we will do this by simply writing a reset function
    `void resetMoves(){`.
    Adding and removing from the vector is cool but what's cooler is actually doing something with them. In order to do that, we are going to tackle the `mazeEndCheck`, `calcMazeStats` and `calcDist` functions.
