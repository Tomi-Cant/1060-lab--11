...
class Player{
    ...
    public:
        ...
        vector<vector<char>> move (vector<vector<char>>& mazeMap, string move){
...
class Maze{
    ...
    public: 
    Maze(){}
    Maze(int len, int wid, char emptySymbol, int cX, int cY, char cSym): height(len), width(wid), coinY(cY), coinX(cX), coinSymbol(cSym) {
        for (int i  = 0; i < len; i++){
            vector<char> temp;
            for (int j = 0; j < wid; j++){
        ...
        mazeMap[coinY][coinX]=coinSymbol;
    }
    ~Maze(){}
...
/*
    Our move function checks if a wall can be placed with the given parameters
...
/*
This function has a problem if given random capitals at any point in the input 
...
void calcMazeStats(vector<string> moves){
    ...
    for (string move : moves){
        ...
        if (move== "c"){
            ...
            cout << "Distance travelled to coin: " << calcDist(deltaX,deltaY) << " Spaces." << endl;
    ...
    if (!collectibleFound){
        cout << "Total distance travelled: " << calcDist(deltaX,deltaY) << " Spaces." << endl;
        
    }else{
        cout << "Distance travelled from coin to end: " << calcDist(deltaX,deltaY) << " Spaces." << endl;
    }
...
*/
void resetMaze(vector<vector<char>>& mazeMap){ // bonus func
    for(vector line : mazeMap){
        for (char space : line){
            space= '*';
        }
...
while (true) {
    ...
    cout << "Attempt failed. Restarting..." << endl;
    return false;
}

Maze selectMaze( char& selectedLevel, Player &player){
    Maze maze1(5,5,'*',3,4,'c');
    Maze maze2(2,7,'*',5,0,'c');
    Maze testMaze(2,2,'*',0,1,'c');
    Maze currMaze;
